...
\item \xxx{Obratnost} (Agility) -- zvyšuje pravděpodobnost zásahu nepřítele a pomáhá se vyhnou nepřátelským útokům,
\item \xxx{Moudrost} (Wisdom) -- je to vlastnost důležitá pro kouzelníky a kněze, určuje rychlost obnovy many,
...
\section{Vstupní data pro herní úrovně -- \ccc{DUNGEON.DAT}}\label{dungeon-objects}
...
\begin{itemize}
\item \xxx{Zeď} -- zajišťuje zobrazování stěn a nelze na ni vstoupit. Pro každou stěnu  (sever, východ, jih, západ -- viz obr. \ref{wall:analyza}) dlaždice \xxx{zdi}
	je určeno, zda může mít tzv. náhodnou dekoraci. Pokud ji může mít, engine podle náhodného generátoru
	určí jaká to bude (případně žádná). Každá strana může obsahovat přepínač, který si může do \xxx{zdi} uložit předměty.
	Pokud má některá strana \xxx{zdi} dekoraci výklenku, jsou v něm zobrazeny předměty uložené ve \xxx{zdi}.
...
\item \xxx{Podlaha} -- Po těchto dlaždicích se hráč běžně pohybuje se svoji skupinou. Obdobně jako u stran \xxx{zdí} může definovat,
...
\item \xxx{Jáma} -- \xxx{Jáma} může být buď otevřená nebo zavřená. Nicméně lze na ni vždy vstoupit a pokud je otevřená 
...
\item \xxx{Schody} -- Po této dlaždici se může hráč dostat o úroveň výše resp. níže.

\item \xxx{Dveře} -- Na těchto dlaždicích je vždy objekt dveře. Na dlaždici lze vstoupit pouze v případě, že jsou dveře
...
\item \xxx{Teleport} -- Tyto dlaždice obsahují objekt teleport, který určuje, které objekty dokáže přenést. Mohou to být
...
\item \xxx{Iluze zdi} -- Tato dlaždice může být buď iluze zdi a nebo otevíratelná zeď. V obou případech je vizuálně neodlišitelná
	od dlaždice typu \xxx{zdi}. V prvním případě je možné na dlaždici vstoupit vkročením do zdi. V druhém případě lze
...
Ke každé instanci objektu existuje unikátní identifikátor
...
Tyto identifikátory se dají chápat jako reference na konkrétní instance objektů \cc{identifier-linklist} a
...
\begin{itemize}
\item \xxx{zbraně} -- lze je vložit šampionům do ruky a pak s nimi mohou bojovat,
\item \xxx{oblečení} -- lze jím obléknout šampiony, a tak zvýšit jejich odolnost,
\item \xxx{svitky} -- obsahují text, který může hráči usnadnit hru, 
\item \xxx{lektvary} -- dělí se na lektvary modifikující šampionovi schopnosti a na lektvary provádějící nějaké
	akce -- např.: výbuch. Každá instance má vlastnost určující sílu efektu.
\item \xxx{kontejnery} -- mohou obsahovat další předměty,
\item \xxx{různé} -- v této kategorii je například jídlo a pak různé předměty, s kterými nelze provádět žádné akce.
\end{itemize}
...
\begin{itemize}
\item \xxx{Dveře} -- můžou být pouze na dlaždici typu \xxx{dveře} a obsahují informace, zda-li jsou dveře rozbitelné a jestli 
	mají tlačítko, kterým je lze otevřít
\item \xxx{Teleport} -- může být pouze na dlaždici typu \xxx{teleport} a definuje cílovou dlaždici a kategorii objektů, které je schopen teleportovat.
\item \xxx{Textové popisky} -- mohou být pouze na dlaždici typu \xxx{zeď} a obsahují odkaz na konkrétní text do textový dat.
\item \xxx{Nepřátelské entity} -- definuje celou skupinu nepřátelský entity na dlaždici, jejich počet, rozmístění na dlaždici a předměty které lze získat jejich zabitím.
\item \xxx{Senzory} -- vytvářejí základní herní mechaniky bludiště. Senzory mají daný číselný typ, pomocí kterého hra 
...
nicméně už není ucelená jako v případě dokumentace pro soubor \ccc{DUNGEON.DAT}. Zároveň 
...
\begin{itemize}
\item \xxx{název} -- textový popis akce ve hře,
\item \xxx{zkušenosti} -- počet zkušeností získaných po provedení akce,
\item \xxx{dovednost} -- identifikátor dovednosti, která získá zkušenosti,
\item \xxx{obrana} -- modifikátor obrany při používání akce,
\item \xxx{výdrž} -- modifikátor výdrže nutné pro provedení akce, 
\item pravděpodobnost provedení akce 
\item \xxx{zranění} -- modifikátor pro výpočet konečného zranění nepřítele, 
\item \xxx{únava} -- doba po kterou nelze provádět žádné akce. 
\end{itemize}
...
\item \xxx{lokace} -- místo, na kterou část těla nebo, do kterého inventáře šampiona lze předmět vložit,
...
Další častí dat uložené v souboru \ccc{GRAPHICS.DAT} jsou vlastnosti kouzel. Pojďme si nejprve přiblížit,
...
\item \xxx{Zdraví} (Base health) -- hodnota použitá pro výpočet zdravý nových nepřátelských entit.
...
Aby se bylo možné soustředit hlavně na samotný návrh enginu \aref{aim-extensibility}, použili jsme framework, s který už jsme měli nějaké zkušenosti.
...
Držák na pochodně \cc{torch-actuator} se skládá ze dvou senzorů. Senzor na první pozici je typu~0,
...
Páku tvoří dva senzory stejného typu~1, který znamená, že senzor lze aktivovat s čímkoliv v hráčově ruce (i s ničím).
První senzor má potom lokální akci rotace senzorů a dekoraci páky dole \cc{lever-actuator}. Druhý senzor má dekoraci s pákou 
nahoře a naopak vzdálenou akci -- prohozeni stavu -- dlaždice \xxx{jámy} na pozici (1,0). Po aktivací prvního senzoru
tedy dojde k rotaci senzorů a změně dekorace na páku dole. Druhý senzor potom odešle zprávu na cílovou dlaždici \xxx{jámy},
...
vzdálenou akci na dlaždici (1,0) - první senzor má deaktivační zprávu a druhý aktivační \cc{button-actuator}.
První senzor má potom opoždění jednu sekundu a druhý šest. Z toho důvodu dojde 1 sekundu po aktivaci přepínače k otevření
\xxx{jámy} na pět sekund.
